...
import { MiddleShapeType, GetNeighbor, DrawArgs, BasicFigureDrawArgs, RotateFigureArgs } from '../../../types/type';
...
export default class QRMiddleShape {
    ...
    constructor({ svg, type }: { svg: SVGElement; type: MiddleShapeType }) {
        this._svg = svg;
        this._type = type;
    }
    ...
    //if rotation === 0 - top right corner is rounded
    _basicCornerThinRounded(args: BasicFigureDrawArgs): void {
        const { size, x, y } = args;
    ...
    _drawCircle({ x, y, size }: DrawArgs): void {
        this._basicCircle({ x, y, size, rotation: 0 });
    }
    ...
    _drawThinRounded({ x, y, size, getNeighbor }: DrawArgs): void {
        const leftNeighbor = getNeighbor ? +getNeighbor(-1, 0) : 0;
        const rightNeighbor = getNeighbor ? +getNeighbor(1, 0) : 0;
        const topNeighbor = getNeighbor ? +getNeighbor(0, -1) : 0;
        const bottomNeighbor = getNeighbor ? +getNeighbor(0, 1) : 0;
        ...
        if (neighborsCount === 2) {
            ...
            this._basicCornerThinRounded({ x, y, size, rotation });
            return;
        }
    ...
    _drawSmooth({ x, y, size, getNeighbor }: DrawArgs): void {
        const leftNeighbor = getNeighbor ? +getNeighbor(-1, 0) : 0;
        const rightNeighbor = getNeighbor ? +getNeighbor(1, 0) : 0;
        const topNeighbor = getNeighbor ? +getNeighbor(0, -1) : 0;
        const bottomNeighbor = getNeighbor ? +getNeighbor(0, 1) : 0;
    ...
    _drawSmoothRounded({ x, y, size, getNeighbor }: DrawArgs): void {
        const leftNeighbor = getNeighbor ? +getNeighbor(-1, 0) : 0;
        const rightNeighbor = getNeighbor ? +getNeighbor(1, 0) : 0;
        const topNeighbor = getNeighbor ? +getNeighbor(0, -1) : 0;
        const bottomNeighbor = getNeighbor ? +getNeighbor(0, 1) : 0;
        ...
        if (!leftNeighbor && !topNeighbor) {
            this._basicCornerThinRounded({ x, y, size, rotation: -Math.PI / 2 });
            return;
        }
